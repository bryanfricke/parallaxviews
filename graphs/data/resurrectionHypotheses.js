...
<body>
  ...
  function showTT(html, [x,y]){ tt.html(html).style('left', (x+12)+"px").style('top', (y+12)+"px").style('display','block'); }
  ...
  function renderRadar(container){
    ...
    const poly = g.append('g').selectAll('path').data(sel, d=>d.id).join('path')
      ...
      .on('mousemove', (e,d)=>{
        ...
        showTT(`<div style="font-weight:600">${d.label}</div>` + scoreLines, d3.pointer(e));
      })
      ...
      sel.forEach(hh=>{
        ...
        g.append('g').selectAll('circle').data(pts.map((p,i)=>({p,i, h:hh}))).join('circle')
          ...
          .on('mousemove',(e,d)=>{
            ...
            const val = weighted(c.id, raw);
            showTT(`<div><strong>${d.h.label}</strong></div><div>${c.label}</div><div>Value: ${fmt(val)} <span style="color:#6b7280">(raw ${fmt(raw)})</span></div><div style="margin-top:4px;color:#6b7280">${Hy[d.h.id].notes[c.id]||''}</div>`, d3.pointer(e));
          })
          .on('mouseleave', hideTT);
  ...
  function renderHeatmap(container){
    ...
    const cells = svg.append('g').selectAll('rect')
      ...
      .on('mousemove', (e,d)=>{
        ...
        showTT(`<div style="font-weight:600">${d.h.label}</div><div>${C[d.c.id].label}</div><div>Value: ${fmt(val)} <span style='color:#6b7280'>(raw ${fmt(raw)})</span></div><div style='margin-top:4px;color:#6b7280'>${Hy[d.h.id].notes[d.c.id]||''}</div>`, d3.pointer(e));
      })
      .on('mouseleave', hideTT);
  }
  ...
  function renderFacts(){
    ...
    const factNodes = Data.facts.map((f,i)=>({type:'fact', id:f.id, label:f.label, index:i}));
    const hypNodes  = hySel.map((h,i)=>({type:'hyp', id:h.id, label:h.label, color:h.color, index:i}));
    const nodes = [...factNodes, ...hypNodes];

    const links = [];
    hySel.forEach(h=>{
      h.covers.forEach(c=>{ links.push({source:c.factId, target:h.id, kind:'covers', strength:c.strength, tip:c.tip}); });
      (h.conflicts||[]).forEach(c=>{ links.push({source:h.id, target:'fact-conflict-'+Math.random().toString(36).slice(2), kind:'conflict', note:c.note, weight:c.weight}); });
    });

    const sim = d3.forceSimulation(nodes)
      .force('charge', d3.forceManyBody().strength(d=> d.type==='fact' ? -100 : -50))
      .force('center', d3.forceCenter((w)/2, h/2))
      .force('x', d3.forceX(d=> d.type==='fact' ? (w*0.3) : (w*0.7)).strength(0.2))
      .force('y', d3.forceY(h/2).strength(0.1))
      .force('link', d3.forceLink(links).id(d=>d.id).distance(l=> l.kind==='covers'? 120: 80).strength(0.2));
    ...
    node.append('circle')
      .attr('r', d=> d.type==='fact' ? 8 : 7)
      .attr('fill', d=> d.type==='hyp' ? (d.color||'#111') : '#0ea5e9')
      .attr('stroke', '#111').attr('stroke-width',0.6);

    node.append('text')
      .attr('x', 12).attr('y', 0).attr('dominant-baseline','middle')
      .attr('font-size', 12)
      .text(d=>d.label)
      .clone(true)
      .attr('stroke','#fff').attr('stroke-width',3).lower();
